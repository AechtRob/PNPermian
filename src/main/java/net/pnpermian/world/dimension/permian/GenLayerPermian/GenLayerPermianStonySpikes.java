...
public class GenLayerPermianStonySpikes extends GenLayer
{
    ...
    public GenLayerPermianStonySpikes(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (k == PERMIAN_STONY_ID)
                {
                    ...
                    if (isStony(l1) && isStony(k2) && isStony(j3) && isStony(i4) & nextInt(36) == 0)
                    {

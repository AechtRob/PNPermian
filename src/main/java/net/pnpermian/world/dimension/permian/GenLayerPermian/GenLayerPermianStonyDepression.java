...
public class GenLayerPermianStonyDepression extends GenLayer
{
    ...
    public GenLayerPermianStonyDepression(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (k == PERMIAN_STONY_ID)
                {
                    ...
                    if (isStony(l1) && isStony(k2) && isStony(j3) && isStony(i4) & nextInt(2) == 0)
                    {
                        aint1[j + i * areaWidth] = PERMIAN_STONY_DEPRESSION_ID;
                    }
                    else
                    {

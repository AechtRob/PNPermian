...
public class BiomePermianDesertEdge extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:permian_desert_edge")
	public static final BiomeGenCustom biome = null;
	public BiomePermianDesertEdge(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
	...
	static class BiomeGenCustom extends BiomePermian {
		public BiomeGenCustom() {
			...
			super(new BiomeProperties("Permian Desert Encroachment").setRainfall(0.0F).setBaseHeight(-0.09F).setHeightVariation(0.04F).setTemperature(2.2F).setRainDisabled().setWaterColor(16767282));
			setRegistryName("lepidodendron:permian_desert_edge");
		...
		protected static final WorldGenRufloriaSingle RUFLORIA_GENERATOR = new WorldGenRufloriaSingle();
		...
		protected static final WorldGenSandyDirtNearWater SAND_WATER_GENERATOR = new WorldGenSandyDirtNearWater();
		...
		public void decorate(World worldIn, Random rand, BlockPos pos)
		{
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 64; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					SAND_WATER_GENERATOR.generate(worldIn, rand, worldIn.getTopSolidOrLiquidBlock(new BlockPos(pos.getX() + j, 0, pos.getZ() + k)).up());
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 16; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					LEAFBLOCK_GENERATOR.generate((BlockBush) BlockBrachyphyllumSapling.block, BlockBrachyphyllumLeaves.block.getDefaultState(), BlockBrachyphyllumLog.block.getDefaultState().withProperty(BlockBrachyphyllumLog.BlockCustom.FACING, EnumFacing.NORTH), worldIn, rand, pos.add(j, l, k), 0, 66);
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 48; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					PLANT_GENERATOR.generate(BlockQuadrocladus.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 0, 255, true);
				}

...
public class BiomePermianLowlandsForestDense extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:permian_lowlands_forest_dense")
	public static final BiomeGenCustom biome = null;
	public BiomePermianLowlandsForestDense(ElementsLepidodendronMod instance) {
		super(instance, 1589);
	}
	...
	static class BiomeGenCustom extends BiomePermian {
		public BiomeGenCustom() {
			...
			super(new BiomeProperties("Permian Dense Wooded Scrub").setRainfall(0.01F).setBaseHeight(0.0025F).setHeightVariation(0.0F).setTemperature(2.2F).setWaterColor(5948010));
			setRegistryName("lepidodendron:permian_lowlands_forest_dense");
		...
		protected static final WorldGenBiarmobaieraTree BIARMOBAIERA_TREE = new WorldGenBiarmobaieraTree(false);
		...
		protected static final WorldGenTreeLog BIARMOBAIERA_GENERATOR = new WorldGenTreeLog(BlockBiarmobaieraLog.block);
		...
		public void decorate(World worldIn, Random rand, BlockPos pos)
		{
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
			{
				...
				for (int j = 0; j < i; ++j)
				{
					...
					int l = rand.nextInt(16) + 8;
					BlockPos blockpos = worldIn.getHeight(pos.add(k, 0, l));
					if (Math.random() > 0.5) {BIARMOBAIERA_GENERATOR.generate(worldIn, rand, blockpos);}
				}
			}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 6; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					LEAFBLOCK_GENERATOR.generate((BlockBush) BlockBiarmobaieraSapling.block, BlockBiarmobaieraLeaves.block.getDefaultState(), BlockBiarmobaieraLog.block.getDefaultState().withProperty(BlockBiarmobaieraLog.BlockCustom.FACING, EnumFacing.NORTH), worldIn, rand, pos.add(j, l, k), 0, 95);
				}
...
//					int k = rand.nextInt(16) + 8;
//					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
//					PLANT_GENERATOR.generate(BlockPolyspermophyllum.block.getDefaultState(), worldIn, rand, pos.add(j, l, k),0,255,true);
//				}

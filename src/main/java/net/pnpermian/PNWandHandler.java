                ...
                if (event.getEntity().getEntityWorld().provider.getDimensionType().getId() == 0) {
                    if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
                        ...
                        if (thePlayer.dimension != DIMID) {
                            ...
                            WorldPermian.BlockCustomPortal.transferPlayerToDimensionPN(thePlayer.server.getPlayerList(), thePlayer, DIMID, WorldPermian.BlockCustomPortal.getTeleporterForDimension(thePlayer, entityIn.getPosition(), DIMID));
                        }
                    }
                    event.setCanceled(true);
                }
